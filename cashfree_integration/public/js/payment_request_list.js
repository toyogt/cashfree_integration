...
frappe.listview_settings['Payment Request'] = {
    onload: function(listview) {
        ...
        function get_user_role_category() {
            let all_roles = frappe.user_roles || [];
            
            let has_accounts_manager = all_roles.includes('Accounts Manager');
            let has_manager = all_roles.includes('Manager');
            let has_director = all_roles.includes('Director');
            
            console.log('=== PAYMENT REQUEST ROLE CHECK ===');
            console.log('All roles:', all_roles);
            console.log('Has Accounts Manager:', has_accounts_manager);
            console.log('Has Manager:', has_manager);
            console.log('Has Director:', has_director);
            
            return {
                is_accountant: has_accounts_manager,
                is_approver: has_manager || has_director,
                all_roles: all_roles
            };
        }
        
        const user_roles = get_user_role_category();
        
        // Exit early if user doesn't have any relevant role
        if (!user_roles.is_accountant && !user_roles.is_approver) {
            console.log('❌ User does not have Accounts Manager, Manager, or Director role');
            return;
        }
        
        console.log('✅ User has relevant roles. Showing appropriate buttons...');
        
        // ====== BULK VERIFY REQUEST (Accounts Manager) ======
        if (user_roles.is_accountant) {
            console.log('Adding: Bulk Verify Request');
        ...
        // ====== BULK APPROVE PAYMENT (Accounts Manager) ======
        if (user_roles.is_accountant) {
            console.log('Adding: Bulk Approve Payment');
        ...
        // ====== BULK QUEUE FOR PAYOUT (Manager/Director) ======
        if (user_roles.is_approver) {
            console.log('Adding: Bulk Queue for Payout');
        ...
        // ====== BULK RETRY PAYOUT (Manager/Director) ======
        if (user_roles.is_approver) {
            console.log('Adding: Bulk Retry Payout');
